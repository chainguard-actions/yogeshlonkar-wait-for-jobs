...
The purpose of this action is to reduce duration of workflow by prestarting dependee jobs. But waiting for jobs in a step will increase Run time, Billable time. 

If the ttl is set more than 15 it will be overridden to 15 minutes. If depdencies requires more than 15 minutes to finish perhaps the dependee job should be split in separate jobs not prestart, but if you still wish to wait more than 15 minutes you can set `no-max-ttl: 'true'`.
